...
# Charms DO circle the Hunter — unlike the weapons, which hold fixed stations so
# a loadout stays readable. Relics are jewellery, not tools: a slow ring of them
# reads as an aura, and there is nothing to aim or plan around, so nothing is
# lost by letting them drift.
